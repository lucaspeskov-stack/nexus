...
export default function Gallery() {
  ...
  return (
    <section id="galeria" className="relative py-20 md:py-28">
      <div aria-hidden className="pointer-events-none absolute inset-0 -z-10 bg-gradient-to-b from-champagne-50 to-champagne-100" />
      ...
      <div className="container-x">
        <Reveal className="mx-auto max-w-2xl text-center">
          ...
          <p className="mt-4 text-lg text-stone">
            A atmosfera acolhedora do {business.shortName}, no coração de {business.neighborhood}.
          </p>
        </Reveal>
        ...
        <div className="mt-12 grid auto-rows-[200px] grid-cols-2 gap-4 sm:auto-rows-[220px] lg:grid-cols-4">
          {gallery.map((img, i) => (
            <Reveal key={i} delay={i * 50} className={`group ${spans[i] ?? ''}`}>
              <div className="relative h-full overflow-hidden rounded-3xl ring-1 ring-champagne-300">
                <SmartImage
                  src={img.src}
                  alt={img.alt}
