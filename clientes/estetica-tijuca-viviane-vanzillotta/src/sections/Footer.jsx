    ...
    <footer className="relative mt-8 bg-ink text-cream-100">
      <div className="container-x grid gap-10 py-14 md:grid-cols-3">
        <div>
          <div className="flex items-center gap-2.5">
            <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-sage-500 font-display text-sm font-bold text-cream-50">
              VV
            </span>
            <span className="font-display text-lg font-semibold tracking-tight text-cream-50">
              Estética <span className="font-normal text-bronze-400">Tijuca</span>
            </span>
          </div>
          <p className="mt-4 max-w-xs text-sm leading-relaxed text-cream-100/70">
            {business.tagline} — com {business.professional}. Autocuidado e resultado em {business.neighborhood}, {business.city}.
          </p>
        ...
        <div>
          ...
          <ul className="mt-4 space-y-2.5">
            {links.map((l) => (
              <li key={l.href}>
                <a href={l.href} className="text-sm text-cream-100/70 transition-colors hover:text-bronze-400">
                  {l.label}
                </a>
              </li>
            ))}
        ...
        <div>
          ...
          <ul className="mt-4 space-y-3 text-sm text-cream-100/70">
            <li className="flex items-start gap-2.5">
              <MapPin size={17} className="mt-0.5 shrink-0 text-bronze-400" />
              <a href={business.maps} target="_blank" rel="noopener noreferrer" className="hover:text-bronze-400">
                {business.address}
              </a>
            </li>
            <li className="flex items-center gap-2.5">
              <MessageCircle size={17} className="shrink-0 text-bronze-400" />
              <a href={business.whatsapp} target="_blank" rel="noopener noreferrer" className="hover:text-bronze-400">
      ...
      <div className="border-t border-white/10">
        <div className="container-x flex flex-col items-center justify-between gap-2 py-6 text-xs text-cream-100/60 sm:flex-row">
          <p>© {new Date().getFullYear()} {business.name} · {business.professional}. Todos os direitos reservados.</p>
