...
export default function Reviews() {
  return (
    <section id="avaliacoes" className="relative overflow-hidden py-20 md:py-28">
      <div aria-hidden className="pointer-events-none absolute inset-0 -z-10 bg-gradient-to-b from-sand-100 via-sand-200 to-sand-200" />

      <div className="container-x">
        <Reveal className="mx-auto max-w-2xl text-center">
          <span className="eyebrow">Avaliações</span>
          <h2 className="mt-4 font-display text-3xl font-semibold tracking-tight text-ink sm:text-4xl">
            Nota máxima entre nossos clientes
          </h2>
          <div className="mt-5 inline-flex items-center gap-3 rounded-full border border-sand-300 bg-sand-50 px-5 py-2.5 shadow-soft">
          ...
          {reviews.map((r, i) => (
            <Reveal key={r.name} delay={i * 60}>
              <figure className="card flex h-full flex-col p-7">
                <Quote size={28} className="text-bronze-400" />
                <blockquote className="mt-3 flex-1 text-sm leading-relaxed text-ink/80">
                  “{r.text}”
                </blockquote>
                <figcaption className="mt-5 flex items-center gap-3 border-t border-sand-300 pt-4">
                  <span className="flex h-11 w-11 items-center justify-center rounded-full bg-gradient-to-br from-bronze-400 to-bronze-600 font-semibold text-sand-50">
                    {r.name.charAt(0)}
                  </span>
