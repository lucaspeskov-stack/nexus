          ...
          {services.map((s, i) => {
            ...
            return (
              <Reveal key={s.title} delay={i * 60}>
                <article className="group card h-full p-7 transition-all duration-300 hover:-translate-y-1 hover:border-emerald-400 hover:shadow-soft-lg">
                  <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-cream-200 text-emerald-600 transition-colors group-hover:bg-emerald-500 group-hover:text-cream-50">
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    {s.tags.map((t) => (
                      <span
                        key={t}
                        className="rounded-full bg-cream-200 px-3 py-1 text-xs font-medium text-emerald-700"
                      >
                        {t}
                      </span>
                    ))}
