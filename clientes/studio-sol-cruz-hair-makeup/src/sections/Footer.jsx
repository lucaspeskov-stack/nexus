      ...
      <div className="container-x grid gap-10 py-14 md:grid-cols-3">
        <div>
          <div className="flex items-center gap-2.5">
            ...
            <span className="font-display text-lg font-semibold tracking-tight text-cream-50">
              Studio <span className="font-normal text-sun-300">Sol Cruz</span>
            </span>
          </div>
          <p className="mt-4 max-w-xs text-sm leading-relaxed text-cream-100/70">
            {business.tagline}. Cabelo e maquiagem em {business.neighborhood}, {business.city}.
          </p>
        ...
        <div>
          ...
          <ul className="mt-4 space-y-2.5">
            {links.map((l) => (
              <li key={l.href}>
                <a href={l.href} className="text-sm text-cream-100/70 transition-colors hover:text-sun-300">
                  {l.label}
                </a>
              </li>
            ))}
        ...
        <div>
          ...
          <ul className="mt-4 space-y-3 text-sm text-cream-100/70">
            <li className="flex items-start gap-2.5">
              <MapPin size={17} className="mt-0.5 shrink-0 text-sun-300" />
              <a href={business.maps} target="_blank" rel="noopener noreferrer" className="hover:text-sun-300">
                {business.address}
              </a>
            </li>
            <li className="flex items-center gap-2.5">
              <MessageCircle size={17} className="shrink-0 text-sun-300" />
              <a href={business.whatsapp} target="_blank" rel="noopener noreferrer" className="hover:text-sun-300">
